...
const UserTypeOrganizer = (props) => {
  ...
  //Get the organizer details by user id
  useEffect(() => {
    const config = OrganizerConfig.getOrganizerByUserId(userId);
  ...
  if (!isLoading && organizerDetails === null) {
    return <BodyText>User not registered for Organizer</BodyText>;
  }
  ...
  return isLoading ? (
    ...
    <div>
      ...
      {organizerDetails.user !== null ? (
        ...
        <>
          <div className={classes.sectionSpace}>
            <BodyHeading>Associated Company Organizer Status</BodyHeading>
            <Divider></Divider>
            <div className={classes.contentSpace}>
              <BodyText>
                Status:{" "}
...
export default UserTypeOrganizer;
