...
exports.updateUser = async (event, context, callback) => {
  ...
  const password = (payLoad.password != null && payLoad.password != "")
    ? payLoad.password
...
exports.screenStatus = async (event, context, callback) => {
  ...
  console.log("userId", userId);
  try {
    const userQuery = "select first_name,last_name,role from users where cognito_id = ?";
    const subscriberQuery = "select subscriber_details_id from subscriber_details where cognito_user_id = ?";
    ...
    if (subscriberRecords.length === 0) {
      ...
      const payerAliasQuery = "select payer_alias_id from payer_alias where created_by = ?";
      const payerRecords = await db.query(payerAliasQuery, [userEmail]);
      const groupMappingQuery = "select group_mapping_id from group_mapping where created_by = ?";
...
exports.signIn = (event, context, callback) => {
  ...
  cognitoUser.authenticateUser(authenticationDetails, {
    onSuccess: (resp) => {
      ...
      response.authorizationToken = resp.idToken.jwtToken;
      ...
      console.log("after getting token:", response);
      console.log("before getting attributes");
      ...
      currentUser.getSession((e, session) => {
        ...
        currentUser.getUserAttributes((err, result) => {
          for (i = 0; i < result.length; i++) {
            ...
            obj["name"] = result[i].getName();
            obj["value"] = result[i].getValue();
    ...
    onFailure: (err) => {
      console.log("Error while authetication:", err);
      return callback(null, {
        statusCode: 502,
        body: JSON.stringify({ error: err.message }),
        headers: {
  ...
  try {
    ...
    await CognitoIdentityServiceProvider.initiateAuth(params)
      ...
      .then((data) => {
        ...
        CognitoIdentityServiceProvider.adminSetUserPassword(changePasswordParams).promise();
        return callback(null, {
          statusCode: 200,
      ...
      .catch((error) => {
        ...
        return callback(null, { statusCode: 502, body: JSON.stringify({ error: error.join(" ") }), headers: headers, });
      });
  } catch (err) {
    console.log("error : ", err);
    return callback(null, {
