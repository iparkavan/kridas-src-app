...
def lambda_handler(event, context):
    ...
    """Grabs transactional level data for eligibility
    ...
    try:
        ...
        # 3. If so, get the list of patients (from the patient_eligibility table instead of patient table. Why? I dont know.)

        patients = db_query(query=f"""SELECT 
                    first_name, last_name, date_of_birth, mrn_patient_account_no, processing_status, comments, patient_eligibility_id
                FROM {rds_db}.patient_eligibility
                WHERE file_upload_id = {transaction_id};""")

        if len(patients) != 0:

            # 4. Create a patients list object, inside, for each patient, 
            #     dict with the patient_eligibility_id, first_name, last_name, dob, mrn, processing_status

            patients = [ [patient_eligibility_id, first_name, last_name, dob.strftime("%m/%d/%Y"), mrn, processing_status, comments, f"{api_url}/benefits/checkEligibility/{transaction_id}/{patient_eligibility_id}"]
                for first_name, last_name, dob, mrn, processing_status, comments, patient_eligibility_id in patients]
