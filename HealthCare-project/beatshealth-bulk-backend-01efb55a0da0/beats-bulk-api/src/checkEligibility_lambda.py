...
def lambda_handler(event, context):
    ...
    """Uploads bulk data for eligibility 
    ...
    try:
        ...
        s3_bucket_uri = f"bulk-eligibility-upload/{user}/{timestamp}_bulk.xlsx"
        ...
        def call_beats_bulk_job_submit_lambda_api(api_url, token, user, file_name, widget_name):
            ...
            payload = {
                "email": user,
                "file_path": f"bulk-eligibility-upload/{user}/{file_name}",
                "file_name": file_name,
        ...
        # url: the URL to make the subsequent call to get the transaction string https://beatshealth.us/benefits/checkEligibility/<transactionId>/
        ...
        out_dict["transactionId"] = file_id # currently it's file_id, but it really should be some hash string
        out_dict["url"] = f"{self_api_url}/benefits/checkEligibility/{file_id}"
