...
exports.handler = async (event) => {
    console.log("******Entering Retry Schedule Lambda  ********");
    const db = makeDb({
        connectionLimit: 10,
    ...
    try {
        var patientQuery = "SELECT * FROM patient where processing_status IN ('Retry')";
        ...
        var patientEligibilityQuery = "SELECT * FROM patient_eligibility where api_status IN ('Retry')";
        ...
        console.log("Error while processing Retry schedule lambda", err);
    } finally {
        await db.close();
    }
};
