...
import { relationshipMenuItems, helathbenefitplanItems } from './constants/claimUploadDataConstants'
...
function EligibilityUpload() {
    ...
    const uploadFile = async (fileToUpload, type) => {
        ...
        let s3MappingPath = 'bulk-eligibility-upload';
    ...
    const downLoadTemplate = async () => {
        ...
        let path = 'bulk-eligibility-upload/template/upload-template.xlsx';
        setLoading(true);
        await Axios({
            method: 'POST',
    ...
    const completeVerification = async () => {
        if (formData.uploadBatchFile === 'uploadBatchFile') {
            ...
            let payload = {
                "email": userId,
                "file_path": `bulk-eligibility-upload/${userId}/${uploadedFileName}`,
            ...
            if(!isError) {
                ...
                try {
                    let config = {
                        method: "post",
                        url: process.env.REACT_APP_BEATS_SAVE_ELIGIBILITY_DATA,
                        headers: {
                            "Content-Type": "application/json",
                            "Access-Control-Allow-Origin": "*",
                            Authorization: "Bearer " + sessionStorage.getItem("idToken"),
                    ...
                    if (responseData.status === 200) {
                        ...
                        setUploadedFileName('');
                        setWidgetName('');
                        history.push("/EligibilitySearch");
                    }
                    setLoading(false);
                } catch (err) {
                    console.log(err);
    ...
    return (
        <div className={classes.root}>
            ...
            <div>
                <b><h5>Eligibility Verification</h5></b>
                ...
                <RadioGroup value={formData.uploadBatchFile} onChange={(e) => formChange('uploadBatchFile', e.target.value)} style={{ marginTop: '10px', marginLeft: '20px' }}>
                    <FormControlLabel value='claimEligibilityRealtime' control={<Radio />} label='Eligibility Realtime' />
