...
import { DROPDOWN_DATA, TABLE_COLUMNS } from './constants/claimDataConstants';
...
function SearchClaim() {
    ...
    const getClaimData = async () => {
        ...
        try {
            let config = {
                method: "post",
                url: process.env.REACT_APP_BEATS_CLAIM_SEARCh,
                headers: {
            ...
            if (responseData && responseData.status === 200 && responseData.data && responseData.data.patientRecords.length > 0) {
                ...
                temp_data.forEach((item, index) => {
                    let temp_procedure_code = item.claimProcedure;
                    item['id'] = `unique_${item.claim_number}_${index}`;
                    for (let j = 0; j < 5; j++) {
                        let procedure_data = temp_procedure_code[j];
                        if (procedure_data) {
                            item['procedureCode' + j] = procedure_data.procedure_code;
                            item['chargeAmount' + j] = procedure_data.charge_amount;
                            item['allowedAmount' + j] = procedure_data.allowed_amount;
                            item['paidAmount' + j] = procedure_data.paid_amount;
                        } else {
                            item['procedureCode' + j] = '';
                            item['chargeAmount' + j] = '';
                            item['allowedAmount' + j] = '';
                            item['paidAmount' + j] = '';
                        }
                    }
    ...
    const handleExport = () => {
        if (claimTableData.length > 0) {
            ...
            selectedData && selectedData.forEach(item => {
                let find = claimTableData.find(obj => obj.id === item);
                if (find) {
                    selected_data_temp.push({
                        patientId: find.patient_id,
                        patientClaimId: find.patient_claims_id
                    })
                }
            })
            let payload = {
                ...
                patientClaimIdList: selected_data_temp
            }
            setLoading(true);
            Axios({
                method: "POST",
                url: process.env.REACT_APP_BEATS_CLAIMS_EXPORT,
                data: payload,
                headers: {
                    "Access-Control-Allow-Origin": "*",
                    Authorization: "Bearer " + sessionStorage.getItem("idToken")
        ...
        selectedData.forEach(item => {
            ...
            if (find) {
                selected_dat_temp.push({
                    ...
                    claim_id: find.patient_claims_id
                })
            }
        });

        try {
            console.log("Delete URL " + process.env.REACT_APP_BEATS_CLAIMS_DELETE);
            let config = {
                method: "post",
                url: process.env.REACT_APP_BEATS_CLAIMS_DELETE,
                headers: {
                    "Content-Type": "application/json",
                    "Access-Control-Allow-Origin": "*",
                    Authorization: "Bearer " + sessionStorage.getItem("idToken"),
