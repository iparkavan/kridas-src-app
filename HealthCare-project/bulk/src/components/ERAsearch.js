...
import { ERA_DROPDOWN_DATA, ERA_TABLE_COLUMNS } from './constants/eraDataConstatnts';
...
function EraSearch() {
    ...
    useEffect(() => {
        ...
        let temp_data = ERA_DROPDOWN_DATA;
    ...
    const getClaimData = async () => {
        ...
        if (formData.searchKey === "transaction_date" || formData.searchKey === "processed_date") {
        ...
        try {
            let config = {
                method: "post",
                url: process.env.REACT_APP_BEATS_ERA_SEARCh,
                headers: {
            ...
            if (responseData && responseData.status === 200 && responseData.data && responseData.data.patientRecords.length > 0) {
                ...
                temp_data.forEach((item, index) => {
                    let temp_adjustment_data = item.adjustmentInfo ;
                    item['id'] = `unique_${item.claims_number}_${index}`;
                    for (let j = 0; j < 5; j++) {
                        let adjustment_data = temp_adjustment_data[j];
                        if (adjustment_data) {
                            item['adjustmentAmount' + j] = adjustment_data.adjustment_amount;
                            item['adjustmentReason' + j] = adjustment_data.adjustment_reason_code;
                            item['adjustmentDescription' + j] = adjustment_data.adjustment_reason_description;
                        } else {
                            item['adjustmentAmount' + j] = '';
                            item['adjustmentReason' + j] = '';
                            item['adjustmentDescription' + j] = '';
                        }
                    }
    ...
    const handleExport = () => {
        if (claimTableData.length > 0) {
            ...
            selectedData && selectedData.forEach(item => {
                let find = claimTableData.find(obj => obj.era_id === item);
                if (find) {
            ...
            let payload = {
                ...
                eraIdList: selected_data_temp
            }
            setLoading(true);
            Axios({
                method: "POST",
                url: process.env.REACT_APP_BEATS_ERA_EXPORT,
                data: payload,
                headers: {
                    "Access-Control-Allow-Origin": "*",
                    Authorization: "Bearer " + sessionStorage.getItem("idToken")
    ...
    const setPatientDetailsdata = (keyName, keyValue) => {
        ...
        if (keyName === 'searchKey' && (keyValue === "transaction_date" || keyValue === "processed_date")) {
            setFormData({
                ...formData,
                ...{ [keyName]: keyValue },
                ...{ ['searchValue']: moment() }
    ...
    //         let find = claimTableData.find(obj => obj.claim_number === item);
    //         if (find) {
    //             selected_dat_temp.push({
    //                 claim_number: find.claim_number
    //             })
    //         }
    //     });
                ...
                <Paper className="pad20 txt-center bx-shadow dbbox" elevation={6}>
                    ...
                    <div style={{ textAlign: 'left', position: 'relative', top: '20px', height: '400px' }}>
                        <DataGrid
                            ...
                            rows={claimTableData}
                            getRowId={(row)=>row.era_id}
                            columns={ERA_TABLE_COLUMNS}
                            rowsPerPageOptions={[]}
                            checkboxSelection
