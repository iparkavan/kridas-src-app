...
function ERAupload() {
    ...
    const getServiceType = async () => {
        await Axios({
            ...
            response.data ? setServiceTypeDataList(response.data) : setServiceTypeDataList([])
        })
    }
    ...
    const getPayerData = async () => {
        await Axios({
            ...
            response.data ? setPayerDataList(response.data) : setPayerDataList([]);
        })
    }
    ...
    const completeVerification = async () => {
        if (formData.uploadBatchFile === 'uploadBatchFile') {
            ...
            } else if (!NPInumber || NPInumber.length !== 10 || !regex.test(NPInumber)) {
