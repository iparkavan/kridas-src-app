...
const PatientInvoiceAdd = (props) => {
  const localClasses = useStyles();
  let history = useHistory();
  const { patientIdParam, treatmentIdParam } = useParams();
  ...
  const onChangeNameValue = (name, value, index) => {
    ...
    switch (name) {
      ...
      case "quantity":
        ...
        //check if the added row is inventory and quantity exceeds the available stock
        if (procedure.detailType === "I") {
          if (procedure.quantity > procedure.stockAvailability) {
            procedure.quantity = procedure.stockAvailability;
          }
  ...
  /*
  If treatment id is passed, we have to load those procedures
  */
  useEffect(() => {
    if (!(treatmentIdParam == null) && treatmentIdParam !== null) {
      dispatchPageState({ field: "isPatientProcedureLoading", value: true });
    }
  }, [treatmentIdParam]);

  useEffect(() => {
    if (isPatientProcedureLoading) {
      getPatientCompletedProcedures(treatmentIdParam);
    }
  }, [isPatientProcedureLoading, treatmentIdParam]);
  ...
  const getPatientCompletedProcedures = (treatmentId) => {
    PatientService.fetchPatientTreatmentByTreatmentId(treatmentId)
      .then((response) => {
        ...
        invoiceInfo.treatmentProcedures.map((proc) => {
          const procedure = {
      ...
      .finally(() => {
        dispatchPageState({ field: "isPatientProcedureLoading", value: false });
      });
  };
  ...
  const handleClose = (event, reason) => {
    ...
    if (submitType === "SAVE") {
      ...
      history.push(
        "/patient/payment/add/" + patientIdParam + "/" + savedInvoiceId
      );
    }
  };
  ...
  const submitForm = (e) => {
    ...
    submitState.invoiceDate = moment(
      submitState.invoiceDate,
      "YYYY-MM-DD HH:mm"
    );

    submitState.invoiceNo = moment().format("YYYYMMDDHHmmss");
    ...
    PatientService.addPatientInvoice(submitState)
      .then((response) => {
        if (response.status === 200) {
          setSavedInvoiceId(Helper.getNumberFromString(response.data.message));
          dispatchPageState({
            field: "openSnackMessage",
            value: true,
          });
