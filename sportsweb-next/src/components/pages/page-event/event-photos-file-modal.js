...
import styles from "../../user/user-pages/user-page-photos/page-photos.module.css";
...
function EventFileDialog({
  ...
  const { data: feedMediaData = undefined, isLoading } = useFeedByMediaId(
    media.media_id,
    currentEvent ? "E" : "U",
    currentEvent ? eventData.eventId : currentUserData?.user_id,
    isModalOpen
  );
  ...
  // const { mutate, isLoading: updateMediaLoading } = useUpdateMedia("event");
          ...
          <ModalBody>
            <Flex direction={{ base: "column", lg: "row" }} align="center">
              ...
              >
                ...
                <Box mt={3} dangerouslySetInnerHTML={{ __html: feedHtml }} />
                ...
                {Boolean(mediaData?.commentcount) && showComments && (
                  <ViewComments
                    ...
                    queryKey={["event-photo", eventData.eventId]}
                  />
                )}
...
export default EventFileDialog;
