  ...
  useEffect(() => {
    ...
    const teamNames = new Array(regCount).fill().map((_, index) => {
      if (eventTeamData[index]) {
        return `${eventTeamData[index].first_name} ${eventTeamData[index].last_name}`;
      } else {
    ...
    setTeams(teamNames);
    setOption(teamNames);
  }, [eventTeamData, isGenerate, setTeams]);
    ...
    >
      ...
      <FieldLayout label="No of Registration">
        <VStack align="flex-start">
          ...
          {+noOfReg > maxReg && (
            <Text color="red">Max Registeration exceeded</Text>
          )}
          {+noOfReg < minReg && (
      ...
      {isGenerate > 0 && (
        <>
          ...
          {onClick && isDuplicate && (
            <Text color="red">Paticipants should not be same</Text>
          )}
        </>
      )}
...
export default EventMatchIndividual;
